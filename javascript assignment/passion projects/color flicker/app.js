...
function pickRdmColor() {
  let rdmColor = Math.floor(Math.random() * colors.length);
  let rdmHex = colors[rdmColor].hex;
  let rdmName = colors[rdmColor].name;
  document.body.style.backgroundColor = rdmHex;
  document.getElementById("color-hex").innerText = rdmHex;
  document.getElementById("color-hex").style.color = rdmHex;
  document.getElementById("color-name").innerText = rdmName;
  document.getElementById("color-name").style.color = rdmName;
}

function setCustomColor() {
  let customHex = document.getElementById("custom-color").value;
  let customColorName = document
    .getElementById("custom-color")
    .value.toUpperCase();

  if (customHex || customColorName != colors.hex || colors.name) {
    document.getElementById("color-name").innerText = "Please enter a valid color name or hex code.";
    document.getElementById("color-hex").innerText = "Please enter a valid color name or hex code.";
  }

  for (let i = 0; i < colors.length; i++) {
    if (customHex == colors[i].hex) {
      document.getElementById("color-name").innerText = colors[i].name;
      document.getElementById("color-hex").innerText = colors[i].hex;
      break;
    }
  }

  for (let i = 0; i < colors.length; i++) {
    if (customColorName == colors[i].name) {
      document.getElementById("color-name").innerText = colors[i].name;
      document.getElementById("color-hex").innerText = colors[i].hex;
      break;
    }
  ...
  document.body.style.backgroundColor = customHex;
  document.getElementById("color-hex").style.color = customHex;
  document.getElementById("color-name").style.color = customHex;
}
